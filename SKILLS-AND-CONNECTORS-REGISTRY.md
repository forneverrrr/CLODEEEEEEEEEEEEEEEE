...
| `thedotmack/claude-mem` | Плагин персистентной памяти между сессиями (SQLite, worker-процесс, MCP) | ❌ пробовал — откатил | Сама идея бьёт точно в нашу главную боль (амнезия между облачными сессиями, ради которой и ведём этот реестр вручную). НО инсталлятор регистрирует всё в `~/.claude/plugins` и `~/.claude-mem` — то есть в домашней папке контейнера, а не в git-репозитории. Это ровно то место, которое, по нашему же опыту (см. HANDOFF, "первая попытка установить скиллы... сама откатилась"), не переживает новый контейнер. В этой облачной среде от него ноль долгосрочной пользы — поставил, убедился, снёс (`npx claude-mem uninstall`). Локально на своей машине (не в облаке) — имеет смысл, там `~/.claude` не эфемерна. |
